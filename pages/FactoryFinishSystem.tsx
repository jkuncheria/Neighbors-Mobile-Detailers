...
const FactoryFinishSystem: React.FC = () => {
  const benefits = [
    ...
    {
      icon: <Wrench className="w-8 h-8 text-[#FF9E2C]" />,
      ...
      <section className="py-24 bg-stone-950 px-4 md:px-16">
        ...
        <div className="mb-20">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <span className="text-[#FF9E2C] font-bold tracking-wider text-sm uppercase">FACTORY FINISH SYSTEM</span>
              <h1 className="text-4xl md:text-5xl font-bold text-white mt-4 mb-6">
                Factory Finish System | Showroom Quality Restoration
              </h1>
              <div className="w-20 h-1 bg-[#FF9E2C] mb-6"></div>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <a href="tel:5126797853" className="border-2 border-[#FF9E2C] text-[#FF9E2C] hover:bg-[#FF9E2C] hover:text-black font-bold py-4 px-8 rounded-xl transition-all flex items-center justify-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {services.map((service, index) => (
              <div key={index} className="flex items-center gap-3">
                <CheckCircle className="w-6 h-6 text-[#FF9E2C] flex-shrink-0" />
        ...
        <div className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {packages.map((pkg, index) => (
              <div key={index} className="border-2 border-stone-800 rounded-2xl p-6 hover:border-[#FF9E2C] transition-colors">
                <h3 className="text-xl font-bold text-white mb-2">{pkg.name}</h3>
                <p className="text-gray-400 text-sm mb-4">{pkg.description}</p>
                <ul className="space-y-2">
                  {pkg.includes.map((item, i) => (
                    <li key={i} className="flex items-center gap-2 text-sm text-gray-300">
                      <CheckCircle className="w-4 h-4 text-[#FF9E2C]" />
                      {item}
                    </li>
                  ))}
                </ul>
        ...
        <div className="bg-gradient-to-r from-stone-900 to-stone-800 rounded-3xl p-12 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/contact" className="bg-[#FF9E2C] hover:bg-yellow-400 text-black font-bold py-4 px-8 rounded-xl transition-all transform hover:scale-105 shadow-lg inline-flex items-center justify-center gap-2">
              Get Your Free Quote
              <ArrowRight className="w-5 h-5" />
            </Link>
            <a href="tel:5126797853" className="bg-stone-800 hover:bg-stone-700 text-white font-bold py-4 px-8 rounded-xl transition-all inline-flex items-center justify-center gap-2">
              <Phone className="w-5 h-5" />
              (512) 679-7853
            </a>
          </div>
