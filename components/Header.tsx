    ...
    <header className="w-full flex flex-col font-sans z-50 relative">
      ...
      <div className="text-xs py-2 px-4 flex justify-center items-center border-b border-stone-800" style={{ backgroundColor: '#1a1a1a' }}>
        <div className="text-center font-semibold text-gray-300">
          ...
          <Link to="/contact" onClick={scrollToTop} className="underline hover:text-[#FF9E2C] text-[#FF9E2C]">Get Quote</Link>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between md:grid md:grid-cols-3 md:items-center">
          ...
          <div className="flex items-center gap-2 md:hidden">
            <Link 
              to="/contact"
              onClick={scrollToTop}
              className="p-2 border-2 rounded-lg" style={{ borderColor: '#FF9E2C', color: '#FF9E2C' }}
            >
              <Mail className="w-5 h-5" />
            </Link>
            <a 
              href="tel:5126797853"
              className="p-2 text-black rounded-lg" style={{ backgroundColor: '#FF9E2C' }}
            >
          ...
          <div className="hidden md:flex items-center space-x-3 md:col-start-3 md:col-end-4 md:justify-end">
            <Link 
              to="/contact"
              onClick={scrollToTop}
              className="flex items-center border-2 hover:text-white font-semibold text-sm px-4 py-2 rounded-lg transition-all" style={{ borderColor: '#FF9E2C', color: '#FF9E2C' }} onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = '#FF9E2C'; e.currentTarget.style.color = 'white'; }} onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = '#FF9E2C'; }}
            >
            ...
            <a 
              href="tel:5126797853"
              className="flex items-center text-black font-semibold text-sm px-4 py-2 rounded-lg transition-all transform hover:scale-105 shadow-md" style={{ backgroundColor: '#FF9E2C' }}
            >
                ...
                {item.subItems && item.subItems.length > 0 ? (
                  ...
                  >
                    <button
                      ...
                      className="w-full md:w-auto flex items-center justify-between py-3 md:py-4 hover:text-[#FF9E2C] transition-colors"
                    >
                      ...
                      <ul className="md:absolute md:top-full md:left-0 md:mt-0 md:shadow-lg md:min-w-[200px] md:rounded-b-lg overflow-hidden" style={{ backgroundColor: '#2a2a2a' }}>
                        {item.subItems.map((subItem) => (
                          <li key={subItem.label} className="border-b last:border-none" style={{ borderColor: '#333' }}>
                            <Link
                              to={subItem.href}
                              className="block py-3 px-4 md:px-6 hover:text-[#FF9E2C] transition-colors" style={{ backgroundColor: 'transparent' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#333'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
                              onClick={() => {
                                setIsMenuOpen(false);
                                setOpenDropdown(null);
                                scrollToTop();
                  ...
                  <a 
                    href={item.href} 
                    className="block py-3 md:py-4 hover:text-[#FF9E2C] transition-colors"
                  >
